...
description: "Native disk-based HNSW index for fast similarity search over vector embeddings"
---
...
for accelerating similarity search over vector embeddings (32-bit and 64-bit float arrays) stored in CypherDB.
...
library, such as `sentence_transformers` in Python to create the embeddings. Alternatively, you can use CypherDB's [`llm`](/extensions/llm)
...
ingested into the CypherDB database.
...
CypherDB allows you to combine vector search with filter predicates by using [projected graphs](/extensions/algo#projected-graphs).
