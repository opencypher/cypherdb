...
description: Extension for running native graph algorithms in CypherDB
---

The `algo` extension allows you to run common graph algorithms such as PageRank,
Connected Components, and Louvain on the graph stored in CypherDB. The algorithms are exposed as
Cypher functions and execute directly inside CypherDB.
...
The graph algorithms run on _projected graphs_ instead of operating directly on CypherDB database tables.
...
CypherDB does not materialize projected graphs in memory, and the corresponding data
...
In CypherDB, both the base graph and projected graphs are directed. For algorithms that are only
