---
title: C++
description: "Use CypherDB embedded graph database in C++ applications with the official C++ API"
---
...
<LinkCard
  ...
  href="https://opencypher.github.io/api-docs/cpp/annotated.html"
/>

## Handling CypherDB output using `getNext()`
...
CypherDB provides two interfaces that enable you to define your own custom scalar and vectorized functions.

### Scalar functions

This API allows user to register ordinary scalar functions which are defined in c++ and use them as CypherDB built-in functions in a query. UDF functions are as efficient as the built-in functions.
Since two c++ primitive types may map to the same Cypher data types(e.g. `int32` can map to both `INT32` or `DATE` in CypherDB), CypherDB provides two overloaded APIs which can eliminate the ambiguity in datatype mapping.

#### Option 1: Scalar function with automatic inference

Create a scalar function by automatically inferring the parameter and result type in CypherDB.
...
* `ARGS`: are the type of the arguments in c++, CypherDB currently support UDF functions with up to 3 parameters.

Parameters:
* `name`: the name of the function to be created in CypherDB (note: function name must be unique).
...
Due to its columnar storage and architecture decisions, CypherDB can execute functions on input data in an
efficient and vectorized manner. In addition to creating scalar UDFs, CypherDB also provides support for
...
#### Vector types in CypherDB

The first type of vector that CypherDB supports is flat vectors.
...
Create a vectorized function by automatically inferring the parameter and result type in CypherDB.
```cpp
template<typename TR, typename... Args>
...
// Note: Users can utilize CypherDB pre-defined FunctionExecutors(UnaryFunctionExecutor, BinaryFunctionExecutor, TernaryFunctionExecutor) to execute operations on parameters without writing code to retrieve the valueVectors.
static void addDate(
    const std::vector<std::shared_ptr<ValueVector>>& parameters, ValueVector& result) {
