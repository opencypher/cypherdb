...
The `postgres` extension allows you to [attach](/extensions/attach/rdbms) PostgreSQL databases to CypherDB.
...
- `alias`: Database alias to use in CypherDB. If not provided, the database name from PostgreSQL will be used.
...
- `schema`: The name of the schema in PostgreSQL to attach. CypherDB attaches to the `public` schema of PostgreSQL by default.
...
For example, the `university` PostgreSQL database can be attached to CypherDB as:
...
### PostgreSQL to CypherDB type mapping

The following table shows the mapping from PostgreSQL's data types to CypherDB's data types:

| PostgreSQL Data Type                      | Corresponding Data Type in CypherDB |
|-------------------------------------------|----------------------------------|
| `bigint (int8)`                             | `INT64`                        |
...
and retrieve its results in CypherDB.
...
You can use the `COPY FROM` statement to import data from a PostgreSQL table into CypherDB.

First, create a `Person` table in CypherDB, with the same schema as the one defined in PostgreSQL:

```cypher
CREATE NODE TABLE Person (name STRING PRIMARY KEY, age INT32);
```

When the schemas are the same, you can copy the data from the external DBMS table to the CypherDB table as:
