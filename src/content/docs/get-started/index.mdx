...
description: "Get started with CypherDB by creating your first graph database and running basic queries"
---
...
export const CODE_BLOCKS = {
    ...
    <version>${version}</version>
        ...
        groovy: `implementation 'com.kuzudb:kuzu:${version}'`,
        kotlin: `implementation("com.kuzudb:kuzu:${version}")`,
        gradle: `plugins {
    id 'java'
    id 'application'
}
...
dependencies {
    implementation 'com.kuzudb:kuzu:${version}'
}`,
},
swift:`import PackageDescription

let package = Package(
    name: "kuzu-swift-example",
    platforms: [
        .macOS(.v11),
        .iOS(.v14),
    ],
...
CypherDB implements a **structured property graph model** and requires a pre-defined schema.
...
CypherDB supports both **on-disk** and **in-memory** modes of operation. The mode is determined
at the time of creating the database, as explained below.

### On-disk database

If you specify a database path when initializing a database, such as `example.kuzu`, CypherDB
will operate in the **on-disk** mode. In this mode, CypherDB persists all data to disk at the given
...
If you omit the database path, by specifying it as `""` or `:memory:`, CypherDB will operate in the **in-memory** mode.
...
Ensure that you have [installed](/installation) CypherDB using the CLI or your preferred client API.
Also download the example CSV files [from our GitHub repo](https://github.com/opencypher/cypherdb/tree/master/dataset/demo-db/csv).

```bash
mkdir ./data/
curl -L -o ./data/city.csv https://raw.githubusercontent.com/opencypher/cypherdb/refs/heads/master/dataset/demo-db/csv/city.csv
curl -L -o ./data/user.csv https://raw.githubusercontent.com/opencypher/cypherdb/refs/heads/master/dataset/demo-db/csv/user.csv
curl -L -o ./data/follows.csv https://raw.githubusercontent.com/opencypher/cypherdb/refs/heads/master/dataset/demo-db/csv/follows.csv
curl -L -o ./data/lives-in.csv https://raw.githubusercontent.com/opencypher/cypherdb/refs/heads/master/dataset/demo-db/csv/lives-in.csv
```

In this example, we will create a graph with two node types, `User` and `City`, and two relationship types, `Follows` and `LivesIn`.

Because CypherDB is an embedded database, there are no servers to set up. You can simply import the
`kuzu` module in your code and run queries on the database. The examples for different client APIs
below demonstrate how to create a graph schema and import data into an on-disk CypherDB database.
...
CypherDB's Java client library is available on [Maven Central](https://central.sonatype.com/artifact/com.opencypher/cypherdb). You can add the following snippet to your `pom.xml` to get it installed:

<Code code={CODE_BLOCKS.java.maven} lang="xml"/>

Alternatively, if you are using Gradle, you can add the following snippet to your build.gradle file to include CypherDB's Java client library:
...
When installing the `kuzu` crate via Cargo, it will by default build and statically link CypherDB's C++
...
fn main() -> Result<(), Error> {
    // Create an empty on-disk database and connect to it
    let db = Database::new("example.kuzu", SystemConfig::default())?;
...
```bash
mkdir kuzu-swift-example
cd kuzu-swift-example
...
// Create an empty on-disk database and connect to it
let db = try! Database("example.kuzu")
...
The CypherDB C++ client is distributed as `so`/`dylib`/`dll+lib` library files along with a header file (`kuzu.hpp`).
...
add_executable(kuzu-cpp main.cpp)

# Find and link kuzu library
find_library(KUZU_LIB kuzu PATHS ${CMAKE_SOURCE_DIR}/lib REQUIRED)
target_link_libraries(kuzu-cpp PRIVATE ${KUZU_LIB})
target_include_directories(kuzu-cpp PRIVATE ${CMAKE_SOURCE_DIR}/include)
```
...
The CypherDB C API shares the same `so`/`dylib` library files with the C++ API and can be used by
...
int main()
{
    // Create an empty on-disk database and connect to it
    kuzu_database db;
    kuzu_database_init("example.kuzu", kuzu_default_system_config(), &db);
...
When using the CypherDB CLI's shell, you can create an on-disk database by specifying a path after
...
:::tip[Congratulations]
You've now created and queried your first graph in CypherDB!
:::
