...
If you're new to Cypher and just getting started with CypherDB, you're in the right place! This tutorial will guide you through the basics of
Cypher, including how to create nodes and relationships, and how to scan, copy and query your data
that's in a CypherDB database.
...
## Working with the CypherDB CLI

CypherDB's command line interface (CLI) is a great way to get started with CypherDB. It allows you to
interact with your database in a terminal and quickly test out your ideas in Cypher.

Let's start by running the CypherDB CLI shell. [Install](/installation/#command-line-shell) the CypherDB CLI
...
The first step in getting your data into CypherDB is creating node and relationship tables. This step
...
The `COPY FROM` command is used to ingest data from various file formats into CypherDB. In this case,
...
In CypherDB, a graph's nodes and relationships are stored in tables. The `MATCH` clause is used to
