...
description: Import data directly from Pandas and Polars DataFrames or PyArrow tables into CypherDB using COPY FROM commands.
---

You can copy from Pandas/Polars dataframes or in-memory Arrow tables directly into CypherDB using the `COPY FROM` command.
This is useful when you are performing data transformations with these libraries and want to quickly ingest
the prepared data into CypherDB.

There are two different ways in CypherDB to import dataframes:
...
because CypherDB uses a strictly-typed schema.
:::
...
You can directly copy from a Pandas dataframe into CypherDB. Both numpy-backed and PyArrow-backed Pandas
...
You can utilize an existing in-memory PyArrow table to copy data directly into CypherDB.
...
We can now scan the DataFrame using the `LOAD FROM` clause in CypherDB in combination with `UNWIND` to
...
aligns well with CypherDB's type system.

#### Define CypherDB schema
...
The above code creates the following tables in CypherDB:
...
Let's proceed to copy the data from Polars into the CypherDB tables.
...
data type of `age` was inferred in Polars as `i8` (signed 8-bit integer), but the CypherDB schema requires
...
That's it! We have successfully copied the relevant data into our CypherDB graph.
...
the data using Polars dataframes and CypherDB. Happy querying!
