---
title: Data types
description: "Primitive and nested data types supported in CypherDB for properties and expressions"
---

import { LinkCard, CardGrid } from '@astrojs/starlight/components';

CypherDB supports a set of primitive and nested data types both for node and relationship properties as
...
ISO/IEC 9834-8:2005, and related standards. CypherDB follows [PostgreSQL's implementation](https://www.postgresql.org/docs/13/datatype-uuid.html)
for the `UUID` format.
...
└───────────┘
```
CypherDB's CLI returns an empty cell to indicate nulls.
...
`[.zzzzzz]` is left out. The default timezone is UTC, and CypherDB stores the timestamp based on the
...
CypherDB follows [DuckDB's implementation](https://duckdb.org/docs/sql/data_types/interval) for the
interval format.
...
4KB in size in CypherDB. The database processes it as binary data because it has no knowledge as to what
...
CypherDB supports two list-like data types: (i) variable-length lists, simply called `LIST`, and
...
:::note[Note]
The JSON data type is not natively available in CypherDB, and requires the [JSON extension](/extensions/json)
:::
