...
description: "Work with JSON data including functions for access, manipulation, and file operations"
---
...
In the following sections, we will first scan the JSON files to query their contents in Cypher, and
...
move the data into a CypherDB table.
...
Because `info` is a nested object, its type in CypherDB is inferred as a `STRUCT` that itself contains
...
You can enforce explicit data types during scanning by using the `LOAD WITH HEADERS` feature.
...
```
We can see that the types inside the `info` STRUCT are now `FLOAT` rather than `DOUBLE`.
```table
┌─────────┬─────────────────────────────┬─────────────────────────────┐
...
The `COPY FROM` statement allows you to copy data from a JSON file into a node or relationship table in CypherDB.
...
```
The syntax `STRUCT( ... )[]` with the square braces at the end represents an array of STRUCTs.
...
When using the `json` extension, keep in mind the following considerations when copying data to CypherDB tables:

1. The order of the keys in the JSON file doesn't need to match the order of the columns defined
...
which is interpreted and parsed as JSON, rather than as a string.

The following example creates a node table `Person` with a `JSON` column `description`,
it then creates two JSON objects in this column using the `to_json` function, and outputs them.
...
This section lists the built-in functions that operate on the JSON data type within CypherDB.

##### `to_json(value:ANY) -> JSON`

Converts `value` of any CypherDB type to JSON.
...
Invalid paths return an empty JSON.
...
If the `json` is a JSON array, returns its length, otherwise returns `0`.
...
Trying to compute the length of a JSON array containing a `null` value results in a length of zero, as
this isn't a valid JSON array.
...
Returns keys of the root `json` object. If the root is not a JSON object, returns an empty list.
...
Returns the type structure of the `json` in CypherDB type notation. Integer widths are automatically inferred from input values.
