...
description: "Learn to use CypherDB's Rust API to analyze social network data with hands-on examples"
---

import { CardGrid, LinkCard } from '@astrojs/starlight/components';
import { Tabs, TabItem } from '@astrojs/starlight/components';

This tutorial will get you started using CypherDB's Rust API to analyze a dataset.
...
Create a new Cargo project with CypherDB as a dependency:
...
Next, [download the zipped data](https://opencypher.github.io/data/tutorial/tutorial_data.zip) and unzip the files.
```bash
...
Before we can start querying the data, we need to create a new CypherDB database
...
`cargo run` will take some time as it [compiles CypherDB from scratch](https://docs.rs/kuzu/latest/kuzu/).
...
Create and connect to an empty CypherDB database:
```rs
...
CypherDB also supports an `in-memory` mode where the database is active only during the execution of the Rust program. For more information, refer to the example in the [create your first graph](/get-started) section.

### Define schema
The first step in building a CypherDB graph is to define a schema. For our example dataset, we need the following node and relationships tables:
```rs
conn.query(
...
Run `create_db.rs` to create the CypherDB database and import the CSV files:
```bash
$ cargo run --bin create_db
...
```
The CSV data has been copied into a new CypherDB database in `social_network.kuzu`.

:::note[Note]
If you run the program again, you'll get an error about duplicated primary keys.
This is because the data has already been imported into the CypherDB database. You
will have to delete the database file (i.e. `rm social_network.kuzu`)
in between runs.
:::
...
Then for each of the following queries, type in the code and run the program as:
```bash
cargo run --bin kuzu_social_network
```
...
## Summary
In this tutorial, we've shown how to use CypherDB's Rust API to import CSV data as a graph and query it using Cypher.

You're now ready to import your own datasets into CypherDB and query them using CypherDB's Rust API!
...
<Tabs syncKey="rust-tutorial-code">
