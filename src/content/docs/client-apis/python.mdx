---
title: Python API
description: "Use CypherDB embedded graph database in Python applications with the official Python API"
---

import { LinkCard } from '@astrojs/starlight/components';
import { Tabs, TabItem } from '@astrojs/starlight/components';

CypherDB provides a Python package that you can install via PyPI. A full list of the available functions and classes
can be found in the Python API documentation, linked below.

<LinkCard
  title="Python API source documentation"
  href="https://opencypher.github.io/api-docs/python"
/>

## Sync and Async APIs

CypherDB provides both a synchronous and an asynchronous Python API. The synchronous API is the default
and is more convenient to use.

Download the [CSV files](https://github.com/opencypher/cypherdb/tree/master/dataset/demo-db/csv) used in the examples below:
```bash
mkdir ./data/
...
The synchronous API is the default and is a common way to work with CypherDB in Python.
...
def copy_data(conn: kuzu.Connection) -> None:
...
def query(conn: kuzu.Connection) -> None:
...
The asynchronous API is useful for running CypherDB in an async context,
such as in web frameworks like FastAPI or cases where you need to concurrently run queries in CypherDB.
...
In Python, CypherDB supports the use of Pandas and Polars DataFrames, as well as PyArrow Tables. This
...
You can scan a Pandas DataFrame, Polars DataFrame, or PyArrow Table in CypherDB using the `LOAD FROM` clause.
Scanning a DataFrame or Table does *not* copy the data into CypherDB, it only reads the data.
...
Copy from a Pandas DataFrame into a CypherDB table using the `COPY FROM` command:
...
Copy from a Polars DataFrame into a CypherDB table using the `COPY FROM` command:
...
Copy from a PyArrow Table into a CypherDB table using the `COPY FROM` command:
...
This section summarizes the type notation used in CypherDB's Python API. Below is a table from Python
types to a CypherDB `LogicalTypeID`, which will be used to infer types via Python type annotations.

|Python type|CypherDB `LogicalTypeID`|
|---|---|
|`bool`|`BOOL`|
|`int`|`INT64`|
|`float`|`DOUBLE`|
...
CypherDB's Python API also supports the registration of User Defined Functions (UDFs),
allowing you to define custom functions in Python and use them in your Cypher queries. This can allow
you to quickly extend CypherDB with new functions you need in your Python applications.

:::caution[Note]
There are performance implications of writing UDFs in Python. Before writing your own UDF, it's
recommended to check if an equivalent Cypher function in CypherDB exists, as native functions run faster
than UDFs.
:::
...
return value in CypherDB, prior to registering the UDF. 
...
for the parameters which are then manually casted to the respective CypherDB type.
...
The second parameter is a built-in native type in CypherDB, i.e., `kuzu.Type.BOOL`. For the first parameter,
we need to specify a string, i.e. `DECIMAL(7,2)` that is then parsed and used by CypherDB
