...
description: Hardware and software requirements for running CypherDB graph database including memory, CPU, and OS specifications.
---

## CLI, C, and C++

The CypherDB CLI and the C and C++ APIs are pre-compiled for:
...
CypherDB's Python API wheel has been pre-compiled for **CPython 3.7 to 3.11**. Compatibility for macOS and Windows is
...
CypherDB's Go API is a wrapper around the C API. The minimum supported Go version is 1.20.

## Swift

CypherDB's Swift API requires Swift 5.9 or later. It supports the following platforms:
- macOS v11 or later
- iOS v14 or later
