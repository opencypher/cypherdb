...
description: "Migrate your Neo4j graph database data directly into CypherDB"
---

import { Tabs, TabItem } from '@astrojs/starlight/components';

The `neo4j` extension allows you to migrate your Neo4j data into a CypherDB database.
It provides the following function:

- `NEO4J_MIGRATE`: Migrate data from Neo4j to CypherDB

:::note[Note]
* Multi-labelled nodes in Neo4j cannot be migrated as CypherDB doesn't support them yet.
* Nodes and relationships without any labels in Neo4j will be skipped during migration.
* Any property with a data type that is not natively supported in CypherDB will be imported into CypherDB as a `STRING`.
:::
...
The following section shows how to migrate _labels_ from Neo4j to _tables_ in CypherDB.

### Importing data from Neo4j

You can use the `NEO4J_MIGRATE` function to import data from Neo4j into CypherDB:
...
- `PASSWORD`: Neo4j password. Leave blank if not needed.
  - Type: `STRING`
- `NODE_LABELS`: A list of node labels to migrate to CypherDB. Use `["*"]` to import all nodes with labels.
  - Type: `STRING[]`
- `REL_LABELS`: A list of relationship labels to migrate to CypherDB. Use `["*"]` to import all relationships with labels.
  - Type: `STRING[]`
...
You can use `SHOW_TABLES()` to verify that the tables have been imported into CypherDB:
```cypher
...
The data from Neo4j is now available in CypherDB and you can start querying them.
