...
path. In CypherDB, both base and projected graphs are directed. Weak connectivity of directed graphs is
defined as the WCC of the corresponding undirected version, i.e., each edge is treated as undirected.

CypherDB implements a parallelized version of the WCC algorithm based on [Ligra](https://jshun.github.io/ligra/docs/examples.html).
...
:::note[Note]
`group_id` is assigned based on CypherDB's internal node offsets. Currently there is no way to assign `group_id` based on node properties.
:::
