...
The `duckdb` extension allows you to [attach](/extensions/attach/rdbms) DuckDB databases to CypherDB.
...
- `alias`: Database alias to use in CypherDB. If not provided, the database name from DuckDB will be used.
  When attaching multiple databases, we recommend using aliases.
- `skip_unsupported_table`: A boolean flag to indicate whether CypherDB should exit with an error or skip
   tables with unsupported data types. Defaults to `False`.
- `schema`: The name of the schema in DuckDB to attach. CypherDB attaches to the `main` schema of DuckDB by default.

For example, the `university.db` DuckDB database can be attached to CypherDB using the alias `uw`:
...
First, configure the [S3 connection](/extensions/s3#configure-the-connection).
...
### DuckDB to CypherDB type mapping

The following table shows the mapping from DuckDB's data types to CypherDB's data types:

| Data type in DuckDB          | Corresponding data type in CypherDB   |
|------------------------------|-----------------------------------|
| `BIGINT`                     | `INT64`                           |
| `BIT`                        | Unsupported                       |
| `BLOB`                       | `BLOB`                            |
...
You can use the `COPY FROM` statement to import data from a DuckDB table into CypherDB.

First, create a `Person` table in CypherDB, with the same schema as the one defined in DuckDB:

```cypher
CREATE NODE TABLE Person (name STRING PRIMARY KEY, age INT32);
```

When the schemas are the same, you can copy the data from the external DBMS table to the CypherDB table simply as:
...
To avoid redundantly retrieving schema information from attached databases, CypherDB maintains a schema cache
...
:::note[Note]
If you have attached multiple databases from different systems, say Postgres, DuckDB, and cypherite,
