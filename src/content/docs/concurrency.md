...
description: "Understanding CypherDB's concurrency model, connection handling, and best practices for parallel access"
---

:::caution[Note]
This section is for users who are interested in gaining a deeper understanding of CypherDB's concurrency model
and some of its limitations. If you are just getting started with CypherDB, you can skip to the [FAQ](#faqs)
section below to get answers to common questions.
:::

In this section, we will go over the basics of how applications connect to a CypherDB database and list some
best practices for concurrent connection handling.

CypherDB supports both **on-disk** and **in-memory** modes of operation.
...
Throughout this documentation, let's suppose you open a CypherDB database file that's on-disk, named
`example.kuzu`.

## Understand connections

### Database and connection objects
Application processes must connect to a CypherDB database in two steps before they can start querying it:
...
CypherDB is an embedded database, i.e., it is a library you embed inside an application process and run as part
of this application process, rather than as a separate process.
You can think of the `Database` object as the CypherDB database software.
Specifically, the `Database` object contains
different components of the CypherDB database software, such as its buffer manager, storage manager, transaction manager, etc.
...
connections from `db2` try to run queries after `db1`'s modification. Therefore, CypherDB will
...
that connects to `example.kuzu`. Say you want to also run the CypherDB CLI alongside your Jupyter notebook,
which also connects to the same `example.kuzu`. When you launch CypherDB CLI and point it to
`example.kuzu`, CypherDB CLI embeds CypherDB and tries to create a `READ_WRITE` Database object. So if your notebook process already
...
read or write queries. This is safe. Every read and write statement in CypherDB is wrapped around a transaction
(either automatically or manually by you). Concurrent transactions that operate on the same database
`example.kuzu` are safely executed by CypherDB's transaction manager (i.e., the transaction manager inside `db`),
again as long as those transactions are issued by connections that were created from the same Database object.
See the documentation on [transactions](/cypher/transaction) for the transactional guarantees that CypherDB provides.

## Example scenarios

Below, we provide some examples and best practices for common scenarios you are likely to run CypherDB under:

### Scenario 1: One process that creates a `READ_WRITE` database
In this scenario, you have a single application process that embeds CypherDB and creates a `READ_WRITE` Database object
...
by CypherDB's transaction manager. Pictorially, this scenario looks as follows:
...
CypherDB and create `READ_ONLY` Database objects that open the same database `example.kuzu`.
...
In certain production settings, you may need to have multiple processes that read and write to the same CypherDB database,
...
1. **One API server process** that embeds CypherDB
...
Note that in terms of processes that embed CypherDB, this design pattern follows
...
we provide a REST-style [CypherDB API server](https://github.com/kuzudb/api-server) powered by Express.js.
...
queries directly on the database by embedding CypherDB in your application.
:::

## Known Issue: CypherDB Explorer not recognizing the file lock permissions

CypherDB ensures that multiple `Database` objects, where one of them is a `READ_WRITE` instance, are not
created by setting some permission flags when opening the database file. This is a lightweight
locking mechanism. However, there is a known issue that CypherDB Explorer is not
...
opened a database and you concurrently start CypherDB Explorer, you should manually ensure that
...
In this section, we address some commonly asked questions related to concurrency and connections in CypherDB.

##### Can I embed CypherDB using both `READ_ONLY` and `READ_WRITE` processes in my application?

No, when embedding CypherDB in your application, you cannot have both `READ_WRITE` and `READ_ONLY` database processes
...
can lead to inconsistencies or data corruption. To avoid this issue, the best practice when embedding CypherDB in your
application is to use design patterns as per one of the scenarios shown pictorially in the sections above.

##### I'm seeing an error related to locks when running CypherDB in a Jupyter notebook. How can I resolve this?

Sometimes, when you are working in a Jupyter notebook and building your CypherDB graph while also trying to
...
for example, a CLI or CypherDB Explorer for graph visualization. In general, it's recommended to only
open the CLI or CypherDB Explorer *after* you have finished any operations to the database (and closing or
...
when operating under in-memory mode. When you create a CypherDB database, there are two things to keep
in mind:
