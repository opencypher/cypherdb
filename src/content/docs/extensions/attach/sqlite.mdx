...
The `sqlite` extension allows you to [attach](/extensions/attach/rdbms) SQLite databases to CypherDB.
...
- `alias`: Database alias to use in CypherDB. If not provided, the database name from SQLite will be used.
...
For example, the `university.db` SQLite database can be attached to CypherDB using the alias `uw`:

```cypher
ATTACH 'university.db' AS uw (dbtype sqlite);
```

### SQLite to CypherDB type mapping

The following table shows the mapping from SQLite's data types to CypherDB's data types:

| SQLite Storage Class / Data Type            | Corresponding Data Type in CypherDB |
|--------------------------------------------|----------------------------------|
| `NULL`                                       | `BLOB`                            |
| `INTEGER`                                    | `INT64`                           |
| `REAL`                                       | `DOUBLE`                          |
...
Set `sqlite_all_varchar_option` to `True` to scan such columns in CypherDB.
...
You can use the `COPY FROM` statement to import data from a SQLite table into CypherDB.

First, create a `Person` table in CypherDB, with the same schema as the one defined in SQLite:
