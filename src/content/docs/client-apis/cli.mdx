...
CypherDB provides a CLI shell through which you can issue Cypher queries or `CALL` commands to query
...
CypherDB supports on-disk, as well as in-memory databases. The functionality of the CLI is identical for both on-disk and in-memory modes.
...
By default, CypherDB's CLI is launched with a maximum buffer pool size of 80% of the available memory.
If you want to launch the CypherDB CLI with a different buffer pool size, you can do so by setting the
...
#### `:help`
Show built-in command list within the CypherDB shell.
...
kuzu> :help
    ...
    Note: you can change and see several system configurations, such as num-threads,
          ...
          See: https://opencypher.github.io/docs/cypher/configuration
```
