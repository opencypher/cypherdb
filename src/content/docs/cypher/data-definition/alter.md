...
If the given column name already exists in the table, CypherDB throws an exception when you try to create it.
To avoid the exception being raised, use the `IF NOT EXISTS` modifier. This tells CypherDB to do nothing when
...
```
This query tells CypherDB to only create the `grade` column if it doesn't exist.
...
If the given column name does not exist in the table, CypherDB throws an exception when you try to drop it.
To avoid the exception being raised, use the `IF EXISTS` modifier. This tells CypherDB to do nothing when
...
```
This query tells CypherDB to only drop the `grade` column if it exists.
