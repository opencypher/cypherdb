---
title: On-disk files managed by CypherDB
description: All the files that are created and managed by CypherDB on disk.
---

## Database files

:::note[Note]
Before version `0.11.0`, CypherDB required the database path to be a directory, and created multiple files
within that directory. This section only describes CypherDB versions starting from `0.11.0`, when it
switched to a single-file format.
:::

CypherDB uses a single-file format for persisting the data to disk. That is, the database is stored in a single file rather than a directory of files.
...
When opening a CypherDB database in the "on-disk" read-write mode, CypherDB creates the database file at the specified
...
The following table lists the different types of files that are created and managed by CypherDB.
...
When you install an official extension by executing `INSTALL <extension_name>`, CypherDB downloads the extension
libraries from the official repo into your home directory at `~/.kuzu/extensions`.
When loading an extension using `LOAD <extension_name>`, CypherDB looks for the extension in the same directory.

### HTTPS local cache

For the `httpfs` extension, when its [local cache](/extensions/httpfs/#local-cache)
is enabled using the command `CALL HTTP_CACHE_FILE=TRUE`, CypherDB saves the cached files under `.cache`
...
The CypherDB CLI records the history of executed Cypher queries in a persistent `history.txt` file.
By default, this file is stored in the `~/.kuzu` directory.
You can configure a different directory for storing the history file using the `--path_history`/`-p` option.
