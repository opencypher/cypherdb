...
ExternalProject_Add(kuzu-prebuilt
    URL https://github.com/opencypher/cypherdb/releases/download/v${version}/libkuzu-linux-x86_64.tar.gz
)
...
<LinkCard
  ...
  href="https://opencypher.github.io/api-docs/c/kuzu_8h.html"
/>
...
### Linking against CypherDB when built from source

See the requirements in https://opencypher.github.io/docs/developer-guide/.
...
FetchContent_Declare(kuzu
    URL https://github.com/opencypher/cypherdb/archive/refs/tags/v0.5.0.zip
...
You need to define `KUZU_STATIC_DEFINE`, and link against the static CypherDB library in `build/src`, as well as `antlr4_cypher`, `antlr4_runtime`, `brotlidec`, `brotlicommon`, `utf8proc`, `re2`, `serd`, `fastpfor`, `miniparquet`, `zstd`, `miniz`, `mbedtls`, `lz4` (all of which can be found in the third_party subdirectory of the CMake build directory. E.g. `build/third_party/zstd/libzstd.a`) and whichever standard library you're using.

## Handling CypherDB output using `kuzu_query_result_get_next()`
