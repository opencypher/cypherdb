...
The `httpfs` extension extends the CypherDB file system by allowing reading from or writing to files hosted on
...
If you're looking to use CypherDB with Azure Blob Storage, see the [Azure extension](/extensions/azure).

:::note[Note]
The S3 and GCS extensions are implemented as _features_ of the `httpfs` extension.

- Over plain HTTP(S), CypherDB only supports reading files.
- For object storage via the S3 or GCS API, CypherDB supports reading, writing, and globbing files.
:::
...
A local file cache is initialized when CypherDB requests the file for the first time.
...
file has changed since the last time CypherDB downloaded it.
:::
