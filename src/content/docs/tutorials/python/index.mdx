...
description: "Learn to use CypherDB's Python API to analyze social network data with hands-on examples"
---

import { CardGrid, LinkCard } from '@astrojs/starlight/components';
import { Tabs, TabItem } from '@astrojs/starlight/components';

This tutorial will get you started using CypherDB's Python API to analyze a dataset.
...
a virtual environment and install CypherDB via `pip`. Refer to this [page](/installation/#python)
...
Before we can start querying the data, we need to create a new CypherDB database and import the CSV files.
...
- The `create_db.py` script will be used to create the CypherDB database and import the CSV files.
...
## Create the database
Clear out the contents of `src/create_db.py` and import CypherDB:
```py
import kuzu
```
Next, create and connect to an empty CypherDB database:
```py
def main() -> None:
...
:::note[Note]
CypherDB also supports an "in-memory" mode where the database is active only during the execution of the Python program.
...
### Define schema
The first step in building a CypherDB graph is to define a schema.
...
### Running the code
Run `create_db.py` to create the CypherDB database and import the CSV files:
```bash
...
This is because the data has already been imported into the CypherDB database, and because
CypherDB is strictly typed, you cannot overwrite data when there are primary key conflicts.
...
## Summary
In this tutorial, we've shown how to use CypherDB's Python API to import CSV data as a graph and query it using Cypher.

You're now ready to import your own datasets into CypherDB and query them usin CypherDB's Python API!
